...
let test_empty () =
  let source = {||} in
  Alcotest.(check bool) "no bindings" true
...
let test_let_struct () =
  ...
  let Struct = struct {}
  |} in
  Alcotest.(check bool) "struct binding" true
  (match (parse_program source) with
  | { bindings = [{value = 
                     {binding_name = {value = Ident("Struct"); _};
                      binding_expr = {value = Struct({fields = []; struct_bindings = []}); _}
  }; _}] } -> true
  | _ -> false)
  ...
  | { bindings = [{value = 
                     {binding_name = {value = Ident("Struct"); _};
                      binding_expr = {value = Function({
                        name = None;
                        params = [{value = FunctionParam({value = Ident("T");_},{value = Reference Ident("Type");_});_}]; 
                        returns = {value = Reference Ident("Type");_}; 
                        exprs = [{value = Struct({fields = []; struct_bindings = []}); _}];
                        _
                      }); _}
  }; _}] } -> true
  | _ -> false)
...
let test_struct_fields source () =
  Alcotest.(check bool) "struct fields" true
  (match (parse_program source) with
  | { bindings = [{value = 
                     {binding_name = {value = Ident("Struct"); _};
                      binding_expr = {value = Struct({fields = [
                        {value={field_name = {value = Ident("a");_}; field_type = {value = Reference Ident("Int257");_}};_};
                        {value={field_name = {value = Ident("f");_}; field_type = {value = FunctionCall({
                          fn = {value = Reference Ident("get_type");_};
                          arguments = [];
                        });_}};_};
  ]; struct_bindings = []}); _}
  }; _}] } -> true
...
let () =
  ...
  run "Syntax" [
    "empty file", [
      test_case "Empty file" `Quick test_empty;
    ];
    "struct", [
      test_case "let syntax for struct" `Quick test_let_struct;
      test_case "let syntax for parameterized struct" `Quick test_let_struct_param;
      test_case "shorthand syntax for struct" `Quick test_struct;
      test_case "shorthand syntax for parameterized struct" `Quick test_struct_param;
